...
var connectionString = 'DefaultEndpointsProtocol=https;AccountName=${storage.name};EndpointSuffix=${environment().suffixes.storage};AccountKey=${storage.listKeys().keys[0].value}'
...
output connectionStringKvRef string = empty(keyVaultName) ? '' : secret.name
